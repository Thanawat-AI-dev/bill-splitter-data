...
(() => {
  ...
  const ANTHROPIC_SETTINGS_KEY = "billsplit_anthropic_settings";
...
and tell the user directly that it couldn't be read, asking for a clearer photo.`;
  ...
  // Anthropic API vision call — the model replies with the JSON object
...
- If the image is not a receipt or cannot be read at all, return the schema above with empty items and a note explaining why.`;
  ...
  let anthropicSettings = loadAnthropicSettings(); // {apiKey, model} — for in-app AI receipt reading
  ...
  function loadAnthropicSettings() {
    try {
      const raw = localStorage.getItem(ANTHROPIC_SETTINGS_KEY);
      return raw ? JSON.parse(raw) : null;
    } catch { return null; }
  }
  function saveAnthropicSettingsToStorage(s) {
    anthropicSettings = s;
    localStorage.setItem(ANTHROPIC_SETTINGS_KEY, JSON.stringify(s));
  }
  ...
  // --- AI receipt photo reading (calls the Anthropic API directly from the
  // browser using a user-supplied key — see stepMenu's settings panel) ---
  ...
  async function callAnthropicReceiptVision(file) {
    if (!anthropicSettings || !anthropicSettings.apiKey) {
      throw new Error("กรุณาใส่ Anthropic API Key ก่อน (ดูหัวข้อ “ตั้งค่า AI อ่านบิลอัตโนมัติ” ด้านล่าง)");
    }
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model: anthropicSettings.model || "claude-opus-4-8",
        max_tokens: 2048,
        messages: [{
          role: "user",
          content: [
    ...
    if (!textBlock) throw new Error("AI ไม่ได้ตอบเป็นข้อความ");
    ...
    body.innerHTML = `
      ...
      <div class="prompt-helper">
        <div>
          ...
          <p>กดปุ่ม “อัปโหลดรูปบิล” แล้วเลือกรูปใบเสร็จ แอปจะส่งรูปให้ Anthropic API อ่านและเติมรายการเมนูให้อัตโนมัติ (ตรวจสอบราคาอีกครั้งก่อนไปขั้นต่อไป) ต้องใส่ Anthropic API Key ของตัวเองก่อนใช้งานครั้งแรก — คีย์จะถูกเก็บไว้ในเบราว์เซอร์นี้เท่านั้น ไม่ถูกส่งไปที่อื่น</p>
        </div>
        <details id="anthropic-settings-details">
          <summary>⚙️ ตั้งค่า AI อ่านบิลอัตโนมัติ (Anthropic API Key)</summary>
          <div class="field"><label>Anthropic API Key</label><input type="password" id="f-anthropic-key" placeholder="sk-ant-..." value="${escapeHtml((anthropicSettings && anthropicSettings.apiKey) || "")}"></div>
          <div class="field"><label>Model</label><input type="text" id="f-anthropic-model" placeholder="claude-opus-4-8" value="${escapeHtml((anthropicSettings && anthropicSettings.model) || "claude-opus-4-8")}"></div>
          <button class="btn ghost sm" id="save-anthropic-key-btn">บันทึกคีย์</button>
          <p style="margin-top:8px;">หาคีย์ได้ที่ <span style="opacity:.8">console.anthropic.com</span> — คีย์จะถูกเก็บไว้ใน localStorage ของเบราว์เซอร์นี้เท่านั้น อย่าแชร์หน้าจอที่มีคีย์ปรากฏอยู่</p>
        </details>
        <div>
  ...
  "vatPercent": 0
    ...
    body.querySelector("#save-anthropic-key-btn").onclick = () => {
      const apiKey = body.querySelector("#f-anthropic-key").value.trim();
      const model = body.querySelector("#f-anthropic-model").value.trim() || "claude-opus-4-8";
      if (!apiKey) { toast("กรุณาใส่ API Key ก่อนบันทึก", true); return; }
      saveAnthropicSettingsToStorage({ apiKey, model });
      toast("บันทึก Anthropic API Key แล้ว");
    };
    body.querySelector("#scan-photo-btn").onclick = () => {
      if (!anthropicSettings || !anthropicSettings.apiKey) {
        toast("กรุณาใส่ Anthropic API Key ก่อน — ดูหัวข้อ “ตั้งค่า AI อ่านบิลอัตโนมัติ” ด้านล่าง", true);
        body.querySelector("#anthropic-settings-details").open = true;
        body.querySelector("#f-anthropic-key").focus();
        return;
      }
    ...
    body.querySelector("#scan-photo-input").onchange = async (e) => {
      ...
      try {
        await withButtonPending(btn, "กำลังให้ AI อ่านรูป...", async () => {
          const parsed = await callAnthropicReceiptVision(file);
          const { added, skipped } = mergeParsedMenu(parsed);
