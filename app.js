...
(() => {
  ...
  function computeSummary(project) {
    ...
    const totalDiscount = Number(project.totalDiscount) || 0;
  ...
  async function renderHome() {
    ...
    if (!isConnected()) {
      root.innerHTML = `
        <div class="card connect-banner">
          <div>
            <h3>ยังไม่เชื่อมต่อ GitHub</h3>
            <p>เชื่อมต่อเพื่อบันทึกโปรเจกต์หารบิลขึ้นรีโพของคุณโดยอัตโนมัติ</p>
          </div>
          <button class="btn primary" id="home-connect-btn">⚙️ เชื่อมต่อ GitHub</button>
        </div>
    ...
    root.innerHTML = `<div class="toolbar-row"><div><div class="section-title">โปรเจกต์หารบิล</div>
      <div class="section-sub">เก็บไว้ที่ ${escapeHtml(settings.owner)}/${escapeHtml(settings.repo)}</div></div>
    ...
    try {
      ...
      idx.entries.forEach((e) => {
        ...
        card.querySelector(".btn-del").onclick = (ev) => {
          ev.stopPropagation();
          confirmModal("ลบโปรเจกต์นี้?", `จะลบไฟล์ของ "${e.name}" ออกจาก GitHub อย่างถาวร`, async () => {
            await deleteProjectFromGithub(e.id);
