...
and tell the user directly that it couldn't be read, asking for a clearer photo.`;
  ...
  async function saveProjectToFirebase(project, summary) {
    ...
    const entry = {
      ...
      ownerUid: cleanProject.ownerUid,
      ownerEmail: cleanProject.ownerEmail || "",
    ...
    root.innerHTML = `
      <div class="auth-shell">
        <div class="card auth-card">
          ...
          <div class="section-sub">ใช้บัญชีอีเมลเพื่อแยกข้อมูลโปรเจกต์ของแต่ละคน</div>
          <form id="auth-form">
            <div class="field"><label>Email</label><input type="text" id="auth-email" autocomplete="email" required></div>
  ...
  async function renderAdmin() {
    ...
    root.innerHTML = `
      <div class="toolbar-row">
        <div><div class="section-title">Admin</div><div class="section-sub">ดูข้อมูลรวมของระบบ</div></div>
  ...
  async function renderHome() {
    ...
    const storageName = isAdmin()
      ? `Admin · Firebase: ${escapeHtml(firebaseSettings.projectId)}`
      : `บัญชี ${escapeHtml(currentUser.email || "")}`;
  ...
  function stepMenu(body, stepIdx) {
    ...
    body.innerHTML = `
      ...
      <div class="section-sub">กรอกชื่อเมนูและราคาตามใบเสร็จ หรือนำเข้าจากไฟล์ JSON ที่ AI อ่านจากรูปบิลให้</div>
      ...
      <div class="prompt-helper">
        <div>
          <strong>ใช้ AI อ่านบิลให้เป็นไฟล์ import ได้เอง</strong>
  ...
  function openSettingsModal() {
    toast("Firebase config ถูกโหลดจากไฟล์ firebase-config.json แล้ว");
  }
  ...
  function initSettingsModal() {
    ...
    const firebaseSave = document.getElementById("firebase-save");
    if (firebaseSave) firebaseSave.onclick = async () => {
    ...
    const firebaseDisconnect = document.getElementById("firebase-disconnect");
    if (firebaseDisconnect) firebaseDisconnect.onclick = () => {
    ...
    const settingsDisconnect = document.getElementById("settings-disconnect");
    if (settingsDisconnect) settingsDisconnect.onclick = () => {
      clearSettings();
      closeSettingsModal();
